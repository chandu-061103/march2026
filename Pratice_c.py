...
if "Sub_Category" in df.columns and selected_sub_category:
    filtered_df = filtered_df[filtered_df["Sub_Category"].isin(selected_sub_category)]
...
if "Sub-Category" in filtered_df.columns:
    sub_category_data = filtered_df.groupby("Sub-Category",as_index=False).agg({"Sales": "sum","Profit": "sum"})
...
if "Discount" in filtered_df.columns and "Profit" in filtered_df.columns:
    ...
    fig5 = px.scatter(
        ...
        x = "Discount",
        y = "Profit",
        color = "Category" if "Category" in
    filtered_df.columns else None,
